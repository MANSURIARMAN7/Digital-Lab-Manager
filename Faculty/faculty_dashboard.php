...
// Faculty ka naam session se nikal rahe hain header mein dikhane ke liye
$faculty_name = isset($_SESSION['name']) ? $_SESSION['name'] : 'Faculty';

// 2. Fetch Subjects directly from MySQL
$faculty_subjects = [];
...
// 3. LIVE DASHBOARD STATS FROM `submissions` TABLE
$total_students = 0;
$pending = 0;
$approved = 0;
$rejected = 0;
...
// 4. FETCH RECENT SUBMISSIONS (Latest 5)
$recent_submissions = [];
$recent_query = "
    SELECT s.*, u.name 
    FROM submissions s 
...
<head>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; }
        ...
        .sidebar-logo {
            width: 105%; /* Thoda zoom in taaki exact edges hide ho jaye */
            height: auto;
        }
        ...
        .dropdown-section { background: white; padding: 15px 25px; border-radius: 12px; margin-bottom: 25px; display: flex; align-items: center; gap: 15px; box-shadow: 0 4px 15px rgba(0,0,0,0.03); }
        .dropdown-section select { padding: 10px; border-radius: 8px; border: 1px solid #cbd5e1; outline: none; min-width: 250px; background: #f8fafc; color: #113460; font-weight: 600; cursor: pointer;}
...
<body>
    ...
    <div class="main">
        ...
        <div class="cards">
            <div class="card" style="border-color: #3b82f6;"><div><h3>Total Students</h3><p><?php echo $total_students; ?></p></div><i class="fas fa-users"></i></div>
        ...
        <div class="table-section">
            <h3 style="color: #0f172a; margin-bottom: 10px;">Recent Submissions</h3>
            <table>
                <tr>
                    <th>Student Name</th>
                    <th>Enrollment No.</th>
                ...
                <?php if (count($recent_submissions) == 0) { ?>
                    <tr><td colspan='4' style='text-align:center; padding: 20px; color: #64748b;'>No recent submissions found for this subject.</td></tr>
                <?php } else { 
